#!/usr/bin/env python3
"""
Run SuperTrend tests for one or more (period, mult) combos over defined windows.
Outputs:
  - results/supertrend_test_results.csv      (per window + config)
  - results/supertrend_trade_results.csv     (per trade + config)

Prereqs:
  * strategies/supertrend.py patched to store ATR on entry (last_atr_on_entry / last_close_on_entry)
  * analyzers/trade_list.py present (from earlier message)
"""
...
SYMBOLS = ["RELIANCE"]   # extend later if needed

# warmup config
BURN_IN_DATE  = "2024-12-01"
WARMUP_FACTOR = 10

# Test windows (add an "All-Period" if you want)
PERIODS = {
...
def run_period(symbol: str,
               ...
               period: int,
               mult: float):
    """Execute Backtrader for a single window + config and collect results."""
    # 1) Load data
    ...
    cerebro.addstrategy(ST, st_period=period, st_mult=mult, eval_start=ts_start)
...
if __name__ == "__main__":
    ...
    for sym in SYMBOLS:
        for cfg in ST_PARAM_GRID:
            for lbl, (s, e) in PERIODS.items():
                run_period(sym, lbl, s, e, period=cfg["period"], mult=cfg["mult"])

    # write outputs
    sum_path   = os.path.join(RESULTS_DIR, "supertrend_test_results.csv")
    trades_path= os.path.join(RESULTS_DIR, "supertrend_trade_results.csv")
